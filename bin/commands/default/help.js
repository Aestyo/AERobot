...
module.exports.run = async (client, message) => {
  const Embed = new Discord.MessageEmbed()
    ...
    .addFields({
      ...
      value:
        "```/help - Affiche cette page``````/ping - Affiche la latence réseau du robot``````/cat - Affiche une mignonne image de chat``````/meme - Affiche un élément de r/Meme``````/clear [Nombre] - Supprime le nombre de message entré``````/spam [Nombre] - Envoie le nombre de message entré``````/tocard - Essaie pour voir```",
      inline: false,
    })
    .addFields({
